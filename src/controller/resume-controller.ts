...
export class ResumeController {
    ...
    static addImage(req: Request, res: Response) {
        ...
        Resume.findOneAndUpdate({ _id: resumeId }, { image_url: path }, { new: true }).populate(`education skills languages
         refrences contact_details employment_history award_achivements interests industrialExposures
          projectDetails strengths weakness objectives`
        ).then((data) => {
            res.status(200).send(data)
        }).catch(err => {
            console.log(err);
    ...
    static getAllResume(req: Request, res: Response) {
        ...
        Resume.find({ user_id: userId})
            .populate(`education skills languages refrences contact_details employment_history 
